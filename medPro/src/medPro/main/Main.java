		...
		try {
			do {
				try {
					...
					case 25:
						System.out.println("--- Relatório Detalhado da consulta ---");
						int idConsultaRelatorio = 0;
						while (true) {
							try {
								System.out.print("Digite o ID da consulta para gerar o relatório: ");
								idConsultaRelatorio = scanner.nextInt();
								scanner.nextLine();
								break;
							} catch (InputMismatchException e) {
								System.out.println(" ID inválido. Por favor, insira um número inteiro.");
						...
						List<String[]> receitas = receitaDAO.buscarReceitaCompletaPorId(idConsultaRelatorio);
						if (receitas == null || receitas.isEmpty()) {
							System.out.println(" Nenhuma consulta encontrada com o ID " + idConsultaRelatorio);
						} else {
